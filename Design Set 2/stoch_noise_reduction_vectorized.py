...
def fsm_tanh_init(Xs):
    current_state = np.array([np.random.randint(0, MAX_STATES)])
    for init_idx in range(MAX_STATES):
        down = Xs[init_idx] == 0 and current_state[-1] > 0
        up = Xs[init_idx] == 1 and current_state[-1] < (MAX_STATES - 1)
        current_state = np.append(current_state, [-1] - down + up)
    return current_state[-1]


def stoch_sorting_min(As, Bs):
    t_1 = Ss * As + Ss_neg * (1 - Bs)
    current_state = np.array([fsm_tanh_init(t_1)])
    for t_idx in range(t_1.size - 1):
        down = t_1[t_idx] == 0 and current_state[-1] > 0
        up = t_1[t_idx] == 1 and current_state[-1] < (MAX_STATES - 1)
        current_state = np.append(current_state, [-1] - down + up)
    t_1 = (current_state >= MAX_STATES // 2)
